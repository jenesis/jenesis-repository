# Session Resume — Downstream/Core Security-Defaults Audit Fan-out
...
and lives on the designated branch `claude/downstream-main-green-audit-hh7dpv` in BOTH repos.

## Mission

Autonomous security-defaults audit + backlog clearing across two repos:
- `raphw/jenesis-repository` (core, PUBLIC) — separate CI pool, fast.
- `raphw/jenesis-downstream` (downstream, PRIVATE) — CI budget is the constraint.
...
3. **EPIC 10b — Principle §10 UX sweep (T10b.1–T10b.5)** → **FULL CONSISTENCY, do all five.**
   ...
   stored state + explicit re-probe), T10b.5 core `` proxy upstream health/index views.
...
editing one file); verify module-scoped; ONE PR per epic. Core tasks get their own PRs
(separate CI pool).
...
**Merge policy:** merge core greens freely (fast pool); merge downstream greens as budget
allows. If downstream CI stops building (budget exhausted): leave locally-verified PRs OPEN, note
it, keep core + local builds going. Squash-merge.
