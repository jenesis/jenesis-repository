# T17.7 — core `Authorization` seam (investigation + plan)

**Ticket (EPIC 17, modularity splits):** turn `build.jenesis.repository.server.Authorization`
into a clean *seam* — an interface/SPI the core exposes — so a downstream or alternate
authorization model can plug in without the core hard-coding one, mirroring the sibling free
...
Two viable designs; recommend **B** for the free repo, with **A** as the downstream-facing follow-up.
...
5. Module-graph churn: new `uses build.jenesis.repository.server.AuthorizationProvider` + `provides`
   ...
   the server package to avoid the downstream depending on the whole server module — likely needed,
...
3. Leave the `@Bean` exactly as is (it already returns via the factories and is already
   `@ConditionalOnMissingBean`). A downstream module now simply defines its own `Authorization`
...
2. **Design A** as a follow-up (add `AuthorizationProvider` + `ServiceLoader` + optional `authz-spi`
   module) once the downstream plug-in point is actually needed — matching `ArtifactStoreProvider`.
