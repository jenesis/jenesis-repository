...
Standing rule for both `jenesis-repository` (free) and `jenesis-downstream`:

1. **Wire formats are tested over WireMock**, not hand-rolled `jdk.httpserver` /
   `com.sun.net.httpserver` stubs or raw `ServerSocket`s.
2. **Containers come from containers**, not the `docker` CLI driven through a
...
4. **Mocks are not over-used**: where a real object or an in-process wire
   (WireMock / containers) is cheap, prefer it; a test must not assert only on
...
Already on WireMock: free 6 modules, downstream 38 modules. A full read-through of
...
- `downstream …/gateway/test/FaultUpstream.java` — the deliberate fault double behind
...
- `downstream …/ai/test/RecordedModelServer.java` — already `WireMockServer`; the
  `jdk.httpserver` import is vestigial (value types only).
- `downstream …/server/test/KeycloakTokenExchangeE2ETest` — no in-process stub; a
  containers real Keycloak (the hermetic `OidcTokenExchangeE2ETest` it references is
  already WireMock).
...
- `downstream …/cli/test/CliDispatcherTest` — ~18 JSON/status API routes with per-test
...
- `downstream …/emulator/test/EmulatorTest` — plain status codes for the load mix (the
  truncation is the client's own request). Needs the alias.
- `downstream …/forwarding/test/{ForwardingTest,CentralPortalTransportTest}` — PUT→201
...
modules, downstream 38 — is already on WireMock; these are the residual tail.
...
- **Downstream**: containers is a real dependency (reached via the
  ...
  - **Container-FIXTURE helpers migrated (DONE this session, real runs):** the Keycloak
    ...
    a bridged containers `GenericContainer` (realm via `withCopyToContainer`,
    mapped-port issuer, discovery-doc wait), and the sigstore rig on a containers
    ...
    (real 7-service stack) all green. (Downstream-side detail; kept here since the rule
    spans both repos.)
  - **Deliberate keeps — `docker` used as a CLIENT tool, not a fixture** (containers
...
- **Free**: the earlier claim that containers "cannot be a module dependency in
  ...
  - **DONE:** the Nexus and Artifactory import fixtures migrated off `docker`-CLI to
    ...
    validated by a real containers run** (boot + `mvn deploy` + import + serve, green).
  ...
  - **Deliberate keeps — `docker` used as a CLIENT tool, not a fixture:**
    ...
    containers cannot issue a host-side `docker push`, so these stay. (`OciImporterTest`
...
- **`SeleniumContainer` (both repos) — migrated (DONE this session, real runs).** It is
  now a containers `GenericContainer` that **deliberately keeps host networking**
  ...
  that agrees with the console's callback and the issuer, which bridged containers
  networking cannot provide (the in-JVM console cannot resolve
  `host.containers.internal`; a bridged browser's `localhost` is not the host's). The
  `containers.exposeHostPorts` / gateway rewrite was therefore rejected in favour of
  keeping the loopback identity — the migration is the containers-managed lifecycle
  ...
  and downstream `ConsoleBrowserTest` + `KeycloakSsoBrowserTest` (136s) green.
...
- **downstream**: the 14 servlet-`Proxy` test files across `server`, `redirect-serve`,
