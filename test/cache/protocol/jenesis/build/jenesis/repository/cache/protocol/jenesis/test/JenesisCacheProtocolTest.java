...
class JenesisCacheProtocolTest {
    ...
    void claims_exactly_two_segments_under_the_prefix() {
        assertThat(protocol.handles("/cache/compile/abc123")).isTrue();
        assertThat(protocol.handles("/cache/compile/")).isFalse();
        assertThat(protocol.handles("/cache//abc123")).isFalse();
        assertThat(protocol.handles("/cache/compile")).isFalse();
    ...
    void declines_every_reserved_segment_so_ownership_does_not_overlap() {
        ...
        assertThat(protocol.handles("/cache/gradle/abc123")).isFalse();
        assertThat(protocol.handles("/cache/maven/p/1/g/a/s/f")).isFalse();
        assertThat(protocol.handles("/cache/bazel/ac/abc123")).isFalse();
        assertThat(protocol.handles("/cache/bazel/cas/abc123")).isFalse();
        // A step that merely starts with a reserved word is still a step.
        assertThat(protocol.handles("/cache/gradle-ish/abc123")).isTrue();
    }

    @Test
    void a_reserved_segment_is_not_an_address_either() {
        assertThat(protocol.address(request("/cache/gradle/abc123", Map.of()))).isEmpty();
    }

    @Test
    void reads_the_address_and_both_headers() {
        assertThat(protocol.address(request("/cache/compile/abc123",
    ...
    void an_unpresented_credential_is_a_complete_address_rather_than_an_empty_answer() {
        assertThat(protocol.address(request("/cache/compile/abc123", Map.of())))
                .hasValueSatisfying(address -> {
    ...
    void a_path_it_does_not_own_reads_as_empty() {
        assertThat(protocol.address(request("/cache/bazel/ac/abc", Map.of()))).isEmpty();
    }

    @Test
    void the_address_deduplicates_because_it_is_a_digest_of_the_bytes() {
        assertThat(protocol.address(request("/cache/compile/abc", Map.of())).orElseThrow().existing())
