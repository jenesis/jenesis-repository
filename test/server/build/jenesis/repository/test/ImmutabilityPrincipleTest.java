 ...
 * build time, across the <em>core</em> module sources. Ported from the downstream
 * {@code ImmutabilityPrincipleTest} (downstream&nbsp;#112) so the core is held to the same standard: the downstream
 * has a structural guard for the mutable-static half of &sect;11, the core (until now) had none &mdash; an audit
 * AMBER. This is that guard for the core, mirroring the downstream matcher and allowlist idiom exactly, only scoped
 * to the core's own {@code source/} tree.
 *
 ...
 * The scan covers all of {@code source/} (every core module, including the server production surface &mdash; read
...
class ImmutabilityPrincipleTest {
    ...
    private static List<Allow> allowlist() {
        ...
        //     (mirrors the downstream Forwarding#credentials / Webhooks#enabled boot-time seams). ---
     ...
     *  matcher never trips on the word "static" in prose or a string literal. Ported from the downstream guard. */
