...
/**
 * Core structural guard (the withhold-egress/materialization invariant EPIC, Audit-23, phase&nbsp;P3): the
 * withhold-change feed cannot be silently unhooked. A retroactive hold reaches a durable, name-bearing derived artifact
 * (the downstream published index) only if every writer of the two durable withhold conventions fires an after-commit
 * signal by construction - so this pins the core choke points that fire it, in the exact mould of its sibling
