 ...
 * jenreg.auth}, blank under {@code auth=false} where the instance is already open), and the security-posture
...
class ConsoleAdviceTest {
    ...
    void read_only_reflects_the_flag_and_defaults_off() {
        ...
        assertThat(advice(Map.of("jenreg.read-only", "true")).readOnly()).isTrue();
        assertThat(advice(Map.of("jenreg.read-only", "false")).readOnly()).isFalse();
    }
    ...
    void anonymous_rights_is_the_trimmed_grant_under_an_enforcing_deployment() {
        ...
        assertThat(advice(Map.of("jenreg.anonymous-rights", "  repository:read  ")).anonymousRights())
