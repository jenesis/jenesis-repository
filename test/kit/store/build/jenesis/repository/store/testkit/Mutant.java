 ...
 * (carrying the mutation mechanism to this second kit).
 *
 * <p><b>Why the kit needs one at all.</b> Every check in {@link PublicationHookContract} states what a hook must do;
 * nothing states that the check <em>could have said otherwise</em>. The first hold-release legs would have passed
...
public enum Mutant {
     ...
     *
     * <p>This is the kit's general vacuity probe and the literal shape once hit: a property whose check a hook that
     ...
     * satisfies. This is the second seam, and it is the converged-over-an-empty-answer shape: a projection that reads
     * as converged <em>over an empty answer</em>, where the comparison the check makes is true of a surface nothing
     * ever wrote to.
     */
