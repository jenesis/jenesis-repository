 ...
 * </ul>
 *
 * <p>Keys live in the shared {@code jenesis.} namespace, so every one of them is also settable as an environment
 * variable in a plain {@code docker run -e}: upper-case it and write an underscore for each dot AND each dash, so
 * {@code jenesis.archive.largest-entry} is {@code JENESIS_ARCHIVE_LARGEST_ENTRY}. That is the spelling
