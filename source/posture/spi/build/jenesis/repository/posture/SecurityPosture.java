 ...
 * grounded in a <em>real</em> {@code jenreg.*} (or Spring) key an operator can actually set, so the fix is copy-and-go.
 *
    ...
    public List<SecurityAdvisory> advise(Configuration config) {
        ...
        if (!config.flag("jenreg.auth", true)) {
            advisories.add(SecurityAdvisory.deployment("jenreg.auth.open", Severity.CRITICAL,
                    ...
                    "jenreg.auth=false serves every request anonymously: anyone on the network can read, "
                    ...
                    "jenreg.auth", "true", DOCS + "#jenreg.auth.open"));
        }
        ...
        if (!config.flag("jenreg.block-private-import-hosts", true)) {
            advisories.add(SecurityAdvisory.deployment("jenreg.importer.ssrf", Severity.WARN,
                    "Import screen is disabled",
                    "jenreg.block-private-import-hosts=false lets an import fetch from private/loopback "
                    ...
                    "jenreg.block-private-import-hosts", "true", DOCS + "#jenreg.importer.ssrf"));
        }
        ...
        if (springProfiles(config).contains("dev")) {
            advisories.add(SecurityAdvisory.deployment("jenreg.profile.dev", Severity.CRITICAL,
                    ...
                    "spring.profiles.active", "<remove dev>", DOCS + "#jenreg.profile.dev"));
        }
        ...
        if (config.flag("jenreg.demo", false) && !config.flag("jenreg.read-only", false)) {
            advisories.add(SecurityAdvisory.deployment("jenreg.demo.writable", Severity.WARN,
                    "The demo instance is writable",
                    "jenreg.demo=true seeds a public demo, but jenreg.read-only is off, so the "
        ...
        //    where jenreg.auth.open above is the advisory and anonymous-rights is redundant). The text names the risk,
        //    never the configured grant value.
        if (config.isSet("jenreg.anonymous-rights") && config.flag("jenreg.auth", true)) {
            if (grantsWriteOrAdmin(config.value("jenreg.anonymous-rights"))) {
                advisories.add(SecurityAdvisory.deployment("jenreg.anonymous.write", Severity.CRITICAL,
                        "Anonymous callers may write or administer",
                        "jenreg.anonymous-rights grants a keyless caller write and/or manage/admin rights, "
                        ...
                        "jenreg.anonymous-rights", "repository:read", DOCS + "#jenreg.anonymous.write"));
            } else {
                advisories.add(SecurityAdvisory.deployment("jenreg.anonymous.enabled", Severity.WARN,
                        "Anonymous read access is enabled",
                        "jenreg.anonymous-rights grants a keyless caller a defined set of rights, so a "
                        ...
                        "Intended for a public read-only mirror; confirm the grant is read-only and pair it with "
                                + "jenreg.read-only=true so the mirror is browsable but immutable.",
                        "jenreg.read-only", "true", DOCS + "#jenreg.anonymous.enabled"));
            }
        }
