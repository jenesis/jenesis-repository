 ...
 *       {@code name()} - so this SPI has no duplicate refusal and no {@code jenesis.repository.<name>=false} toggle. A
 ...
 *   <li><b>Error visibility (&sect;9).</b> A throw is <b>not contained</b>: it propagates out of
 *       {@link PostureReport#from} and degrades the whole collected report, taking the console header badge (rendered
 *       on every console view) and {@code GET /api/posture} with it. An advisor that cannot evaluate a condition must
 *       therefore answer - with an advisory naming what it could not determine, or with nothing - rather than throw.
 *       Nothing else here may be swallowed either: an advisory an advisor decides not to raise is a decision, and
 *       silence must mean the condition does not hold.</li>
 ...
 *       and are unique across advisors - a duplicate id is a collision between modules, not a merge.</li>
