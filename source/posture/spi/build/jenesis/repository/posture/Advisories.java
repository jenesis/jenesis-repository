...
/**
 * The naming grammar every security-posture advisory id shares - the same {@code jenreg.<feature>.<signal>} convention
 * the {@code configuration} keys and the observation {@code Signals} use, kept in one place so an advisory id reads like
 * the setting it is about ({@code jenreg.auth.open} sits beside {@code jenreg.auth}). An id is validated at
...
public final class Advisories {
    ...
    public static String require(String id) {
        if (!valid(id)) {
            throw new IllegalArgumentException("Not a jenreg.<feature>.<signal> advisory id: " + id);
        }
        return id;
    }
}
