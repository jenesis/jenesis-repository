...
free repository console (`source/ui`) and is the base the downstream console extends, so the two products speak one
...
## 5. How the downstream console extends this base

The intended model is that the downstream console reuses this base rather than re-vendoring it — serving the same
`app.css` and `th:replace`-ing the `base.html` fragments — and applying the shell across every downstream screen is
...
So the downstream console currently **still re-vendors** these assets; it does not yet depend on this module. The
