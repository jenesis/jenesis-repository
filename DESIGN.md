...
The downstream console is a separate module that reuses this base rather than re-vendoring it: it depends on this
UI module (a `requires build.jenesis.repository.ui` JPMS edge), serves the same `app.css`, and `th:replace`-es the
`base.html` fragments. Applying the shell and component set across every downstream screen — and de-duplicating the
